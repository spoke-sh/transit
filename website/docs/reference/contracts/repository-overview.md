---
title: "Why Transit"
sidebar_label: "Why Transit"
description: "Why Transit exists and what kind of system it is trying to be."
---
# Why Transit
...
For hosted external workloads, the intended boundary is intentionally thin:

- consumers target one `transit-server` endpoint
- acknowledgements expose the literal durability that was actually reached
- `request_id` values and remote error envelopes stay visible end to end
- domain-specific schema and policy stay outside Transit core

For Rust consumers, `transit-client` is the intended client boundary. It should carry hosted operations, response and error types, and replay-driven projection helpers instead of forcing downstream systems to invent a parallel client layer.

Cutover should move authority to the hosted server boundary rather than preserve a second local authority store in parallel.

## Explore Next

- [System Model](../../foundations/system-model.mdx) for the high-level storage and lineage model
- [Architecture](./architecture.md) for the engine, runtime, and storage boundaries
- [Configuration](./configuration.md) for deployment shapes and durability settings
- [Integrity](./integrity.md) and [Materialization](./materialization.md) for verifiable history and derived state
- [Communication](./communication.md), [AI Workload Model](./ai-traces.md), and [AI Artifacts](./ai-artifacts.md) for reference workloads
- [Proof And Benchmarks](./evaluations.md) for the evidence bar behind public claims
