...
| hosted operator surface | `transit server run`, `transit streams`, `transit produce`, `transit consume`, `cargo run -p transit-client --example proof` | shared-engine server semantics over a remote boundary, including the Rust client surface | current remote operator surface is still single-node |
...
- [Foundations](../reference/contracts/repository-overview.md)
