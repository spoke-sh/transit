...
| hosted operator surface | `transit server run`, `transit streams`, `transit produce`, `transit consume` | shared-engine server semantics over a remote boundary | current remote operator surface is still single-node |
| storage and recovery | `transit proof tiered-engine`, `transit proof warm-cache-recovery`, `transit storage probe` | tiered publication, cold restore, restart-from-remote, and effective-config verification | `storage probe` verifies the current local/filesystem guarantee, not cloud-provider durability |
...
`transit storage probe` is intentionally narrow. It verifies the effective local Transit storage configuration for the current runtime shape and reports the explicit guarantee it can actually prove today.
