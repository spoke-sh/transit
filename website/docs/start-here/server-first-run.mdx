...
```bash
transit proof networked-server --root target/transit-docs/networked
```
...
```bash
transit proof hosted-authority --root target/transit-docs/hosted-authority
```
...
```bash
transit server run --root target/transit-docs/server
```
