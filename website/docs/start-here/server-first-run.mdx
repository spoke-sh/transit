...
That proof is a storage proof, not a transport proof. It shows that sealed
segments may be stored compressed while replay and hosted reads still return
normal Transit records.
...
- `active.segment` and `state.json` remain mutable working files
- published segment objects and manifest snapshots are immutable once written
- `frontiers/latest.json` is the latest-discovery pointer for published state
- the same `streams/<id>/segments|manifests|frontiers/latest.json` shape exists locally and in the remote object-store namespace
...
That explicit creation step is how you set root-stream actor, reason, labels,
and retention policy. For quick producer checks, `transit produce` also creates
a missing root stream with CLI lineage metadata before appending the first
record.
...
Query hosted streams through the SQL surface. Stream ids are registered as table
names, so quote ids that contain punctuation:

```bash
transit sql \
  --server-addr 127.0.0.1:7171 \
  -c 'SELECT COUNT(*) AS count FROM "demo/main"'
```
...
transit sql \
  --server-addr 127.0.0.1:7171 \
  -c 'SELECT id, LAST(balance) AS balance FROM accounts GROUP BY id'
```
...
The same rule applies to compression: sealed segments may be stored compressed,
but hosted reads still observe normal Transit records.
