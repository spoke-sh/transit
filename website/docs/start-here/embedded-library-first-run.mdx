...
```bash
cargo run -p transit-cli --bin transit -- local-engine-proof --root target/transit-docs/local
```

Run the tiered publication/restore proof:

```bash
cargo run -p transit-cli --bin transit -- tiered-engine-proof --root target/transit-docs/tiered
```
