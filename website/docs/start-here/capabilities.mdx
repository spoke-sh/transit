...
| hosted operator surface | `transit server run`, `transit streams`, `transit produce`, `transit consume`, `transit server append`, `transit proof hosted-authority` | shared-engine server semantics over a remote boundary, including the Rust client surface, single-stream batch append, and explicit transport-timeout tuning for hosted proofs | current remote operator surface is still single-node |
