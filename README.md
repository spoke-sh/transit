...
- an initial shared-engine server bootstrap that can open the same local engine, bind a daemon listener, shut down deterministically, and serve provisional remote append/read/tail/branch/merge/lineage-inspection operations without introducing a second storage path
...
- a server daemon exposing the same semantics over a network API, starting from the shared-engine bootstrap and provisional remote append/read/tail/branch/merge/lineage-inspection support
