...
|----|-------------|--------------|
| MG-01 | Make hosted Transit the authoritative append, replay, and projection substrate for external workload messages so downstream control planes such as Spoke Hub stop treating local embedded storage as the source of truth for domain-owned state. | board: VGh59soBt |
...
- Do not bake consumer-owned business policy into Transit core. Downstream workloads such as Spoke auth/account/session may shape examples and proof fixtures, but domain rules and schema ownership stay in the consumer.
