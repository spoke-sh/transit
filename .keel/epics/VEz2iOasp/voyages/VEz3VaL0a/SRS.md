# Python Client Library And Proof - SRS

## Summary

Epic: VEz2iOasp
Goal: Deliver a Python client covering append, read, tail, branch, merge, and lineage inspection with an end-to-end proof script against a local server.
...
- [SCOPE-04] Client libraries for languages beyond Python.
...
|----|-------------|-------|--------|--------------|
| SRS-01 | Implement `TransitClient.tail_open()` plus `poll()`, `grant_credit()`, and `cancel()` so the Python client supports explicit tail lifecycle with credit-based delivery. | SCOPE-01 | FR-02 | test + proof |
...
| SRS-03 | Implement `TransitClient.lineage()` so the Python client can inspect the branch and merge DAG exposed by the server. | SCOPE-01 | FR-03 | test + proof |
...
| SRS-05 | Validate the existing `create_root()`, `append()`, `read()`, `create_branch()`, and `create_merge()` methods through an end-to-end proof against a local server. | SCOPE-01 | FR-01 | proof |
| SRS-06 | Deliver a proof script at `clients/python/proof.py` that exercises tail and lineage in addition to the existing core operations, reports pass/fail per operation, and exits non-zero on failure. | SCOPE-02 | FR-04 | proof |
...
| SRS-NFR-02 | The proof script must be runnable from the repo without external service dependencies beyond a locally started transit server. | SCOPE-02 | NFR-02 | proof |
