...
3. Install the git hooks with `keel hooks install` so the pre-commit hook enforces `just quality` and `just test`, while the commit-msg hook appends `keel doctor --status`.
